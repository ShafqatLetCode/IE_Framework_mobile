	...
	public void afterInvocation(IInvokedMethod method, ITestResult testResult) {
		if (method.getTestMethod().getMethodName().contains("Precondition")) {
			if (testResult.getStatus() == ITestResult.SUCCESS) {
				...
				ContextManager.getExtentReportForPrecondition().log(Status.PASS,
						method.getTestMethod().getMethodName() + "Test Passed");
			}

			if (testResult.getStatus() == ITestResult.FAILURE) {
				System.out.println("FAIL");
				ContextManager.getExtentReportForPrecondition().log(Status.FAIL,
...
//					ContextManager.getExtentReportForPrecondition().addScreenCaptureFromBase64String(ScreenshotUtils.base64conversion(ContextManager.getAndroidDriver()), method.getTestMethod().getMethodName());
			...
			if (testResult.getStatus() == ITestResult.SKIP) {
				...
				ContextManager.getExtentReportForPrecondition().log(Status.SKIP,
	 ...
	 * 
	 * if(method.getTestMethod().getMethodName().contains("Precondition")) { //
	 * report = ContextManager.getExtentReportForClassMethods().createNode(method.
	 * getTestMethod().getMethodName(), ""); //
	 * ContextManager.setExtentReportsForPrecondition(report); } }
	 * 
	 * public void afterInvocation(IInvokedMethod method, ITestResult testResult) {
	 * if(method.getTestMethod().getMethodName().contains("Precondition")) { if
	 * (testResult.getStatus() == ITestResult.SUCCESS) { System.out.println("PASS");
	 ...
	 * "Test Failed \n"+testResult.getThrowable().getMessage()); try { //
	 * ContextManager.getExtentReportForPrecondition().
	 * addScreenCaptureFromBase64String(ScreenshotUtils.base64conversion(
	 * ContextManager.getAndroidDriver()), method.getTestMethod().getMethodName());
	 * } catch (Exception e) { e.printStackTrace(); } }
	 * 
	 * if (testResult.getStatus() == ITestResult.SKIP) { System.out.println("SKIP");
