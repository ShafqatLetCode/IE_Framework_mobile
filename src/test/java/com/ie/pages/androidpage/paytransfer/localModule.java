...
public class localModule extends CommonAppiumTest {
	...
	@ElementDescription(value = "Select Previous Month")
	@AndroidFindBy(xpath = "//android.widget.ImageButton[@content-desc='Previous month']")
	private MobileElement SelectPrevMonth;
	...
	public void clickOnLogOutButton() throws Exception {
		try {
			...
			obj_handleexception.throwHandleException("FUNCTIONAL_EXCEPTION", " Failed to Click On 'Log Out' Button ",
					e);
		} catch (Exception e) {
	...
	public void VerifyFastServiceInReview() throws Exception {
		try {
			gestUtils.scrollUPtoObject("text", "TRANSFER NOW", TransferNowBtn);
			Asserts.assertTrue(FastTransactionService.isDisplayed(), "Fast Service not available in review.");
		} catch (HandleException e) {
	...
	public void enterAmountAndVerifySgdCurrency(String Amount) throws Exception {
		try {
			Asserts.assertEquals(getTexOfElement(sgdFieldText), CommonTestData.SGD_CURRENCY_LABEL.getEnumValue(),
					"'Currency' is not Matching");
	...
	public void verifyTransferToOtherBankPageHeader(String expectedText) throws Exception {
		try {
			...
			obj_handleexception.throwHandleException("VERIFYHEADER_EXCEPTION",
					" Failed to verify 'Transfer To Other Bank' page header ", e);
		} catch (Exception e) {
			obj_handleexception.throwException("VERIFYHEADER_EXCEPTION",
					" Failed to verify 'Transfer To Other Bank' page header ", e);
		}
	...
	public void verifyTransferredMessage(String expectedText) throws Exception {
		try {
			...
			obj_handleexception.throwHandleException("VERIFYHEADER_EXCEPTION",
					" Failed to verify 'Transferred' Message ", e);
		} catch (Exception e) {
	...
	public void SelectFutureDate() throws Exception {
		try {
			...
			if (isElementEnable(StaticDate)) {
				...
				// when static date is getting enabled but after coming on next month calendar,
				// we are getting static date disabled
				// because sometimes calendar opening as per current date.
				if (!isElementEnable(StaticDate))
					clickOnElement(SelectPrevMonth);
				...
				if (todayDate[0].equals("30") || todayDate[0].equals("31") || todayDate[0].equals("28")
						|| todayDate[0].equals("29")) {
				...
				String ActualSelectedDate = getTexOfElement(TransferDateTextElement);
				// Asserts.assertEquals(ActualSelectedDate, ExpectedDate, "Selected Date is not
				// Matching");
